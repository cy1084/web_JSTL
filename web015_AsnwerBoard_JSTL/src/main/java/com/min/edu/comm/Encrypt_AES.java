...
public class Encrypt_AES {
	...
	//PKCS padding  패딩
	/*
	 * AES는 대칭 암호화 알고리즘으로, 데이터를 블록 단위로 암호화합니다. 
	 * CBC 모드는 암호화 모드 중 하나로, 각 블록을 이전 블록의 암호문과 연산하여 암호화합니다. 
	 * PKCS 패딩은 블록 크기로 나누어지지 않는 데이터를 변환하기 위한 패딩 방식입니다. 
	 * 이를 통해 AES-CBC에서는 데이터를 안전하게 암호화하고 복호화할 수 있습니다.
	 */
	private String transformation = "AES/CBC/PKCS5PADDING";
	
	//키생성 알고리즘을 통해서 구성되어야한다
	private String key = "a7734567890123456789012345678901";
	
	//CBC모드의 초기화 백터(값) 16자를 가지고 있음 보안강화를 위해서 사용
	...
	public String decrypt(String cryptedTxt) throws NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException, InvalidAlgorithmParameterException, IllegalBlockSizeException, BadPaddingException, UnsupportedEncodingException {
		Cipher cipher = Cipher.getInstance(transformation);
		SecretKey keySec = new SecretKeySpec(key.getBytes(), "AES");
		IvParameterSpec ivParameterSpec = new IvParameterSpec(iv.getBytes());
